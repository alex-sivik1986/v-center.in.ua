...
//var_dump($this->request->post); die;
	...
	protected function getForm() {
		...
		foreach ($option_values as $option_value) {
			if (is_file(DIR_IMAGE . $option_value['image']) || is_file(DIR_IMAGE . $option_value['image2'])) {
				$image = $option_value['image'];
				$image2 = $option_value['image2'];
				$thumb = $option_value['image'];
				$thumb2 = $option_value['image2'];
			} else {
				$image = '';
				$thumb = 'no_image.png';
				$image2 = '';
			...
			foreach ($options as $option) {
				...
				if ($option['type'] == 'select' || $option['type'] == 'radio' || $option['type'] == 'checkbox' || $option['type'] == 'image') {
					...
					foreach ($option_values as $option_value) {
						...
						$option_value_data[] = array(
							'option_value_id' => $option_value['option_value_id'],
							'name'            => strip_tags(html_entity_decode(!empty($option_value['long_name'])?$option_value['long_name']:$option_value['name'], ENT_QUOTES, 'UTF-8')),
							'image'           => $image
						);
					}
