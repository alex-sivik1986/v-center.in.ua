...
class ModelCatalogInformation extends Model {
	public function addInformation($data) {
		...
		foreach ($data['information_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "information_description SET information_id = '" . (int)$information_id . "', language_id = '" . (int)$language_id . "', title = '" . $this->db->escape($value['title']) . "', description = '" . $this->db->escape($value['description']) . "', description2 = '" . $this->db->escape($value['description2']) . "', meta_title = '" . $this->db->escape($value['meta_title']) . "', meta_h1 = '" . $this->db->escape($value['meta_h1']) . "', meta_description = '" . $this->db->escape($value['meta_description']) . "', meta_keyword = '" . $this->db->escape($value['meta_keyword']) . "'");
			
			
			foreach ($data['chart-heading'] as $language_id => $value) {
			$this->db->query("UPDATE " . DB_PREFIX . "information_description SET chart_headings = '" . $this->db->escape(serialize($data['chart-heading'])) . "' WHERE language_id = '" . $language_id . "' AND information_id = '" . (int)$information_id . "'");
		}
		foreach ($data['size-data'] as $language_id => $value) { 
			$this->db->query("UPDATE " . DB_PREFIX . "information_description SET size_data = '" . $this->db->escape(serialize($data['size-data'])) . "' WHERE language_id = '" . $language_id . "' AND information_id = '" . (int)$information_id . "'");
		}
		foreach ($data['chart-name'] as $language_id => $value) {
			$this->db->query("UPDATE " . DB_PREFIX . "information_description SET chart_name = '" . $this->db->escape(serialize($data['chart-name'])) . "' WHERE language_id = '" . (int)$language_id . "' AND information_id = '" . $information_id . "'");
		}
		}
    ...
    public function edit($id) {
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "information_description WHERE information_id = '" . (int) $id . "'");

        foreach ($query->rows as $result) {
            $sizechart['information_id'] = $result['information_id'];        
            $sizechart['chart_headings'] = $result['chart_headings'];
            $sizechart['size_data'] = $result['size_data'];
			$sizechart['chart_name'] = $result['chart_name'];
        }
