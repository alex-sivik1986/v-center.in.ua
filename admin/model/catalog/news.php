...
class ModelCatalogNews extends Model {
	...
	public function addNews($data) {
		...
		foreach ($data['chart-heading'] as $language_id => $value) { 
			$this->db->query("UPDATE " . DB_PREFIX . "news_description SET chart_headings = '" . $this->db->escape(serialize($data['chart-heading'])) . "' WHERE language_id = '" . $language_id . "' AND news_id = '" . (int)$news_id . "'");
		}
		
		foreach ($data['size-data'] as $language_id => $value) { 
			$this->db->query("UPDATE " . DB_PREFIX . "news_description SET size_data = '" . $this->db->escape(serialize($data['size-data'])) . "' WHERE language_id = '" . $language_id . "' AND news_id = '" . (int)$news_id . "'");
		}
		foreach ($data['chart-name'] as $language_id => $value) {
			$this->db->query("UPDATE " . DB_PREFIX . "news_description SET news_id = '" . (int)$news_id . "',  chart_name = '" . $this->db->escape(serialize($data['chart-name'])) . "' WHERE language_id = '" . (int)$language_id . "' AND news_id = '" . $news_id . "'");
		}
		}
	...
	public function edit($id) {
        ...
        foreach ($query->rows as $result) {
            $sizechart['news_id'] = $result['news_id'];        
	...
	public function editNews($news_id, $data) {
		...
		foreach ($data['chart-name'] as $language_id => $value) {
			$this->db->query("UPDATE " . DB_PREFIX . "news_description SET  chart_name = '" . $this->db->escape(serialize($data['chart-name'])) . "' WHERE language_id = '" . (int)$language_id . "' AND news_id = '" . $news_id . "'");
		}
		}
