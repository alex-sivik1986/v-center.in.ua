	...
	public function addOption($data) {
		...
		if (isset($data['option_value'])) {
			foreach ($data['option_value'] as $option_value) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "option_value SET option_id = '" . (int)$option_id . "', image = '" . $this->db->escape(html_entity_decode($option_value['image'],  ENT_QUOTES, 'UTF-8')) . "', long_name = '" . $option_value['long_name'] . "', image2 = '" . $this->db->escape(html_entity_decode($option_value['image2'],  ENT_QUOTES, 'UTF-8')) . "', sort_order = '" . (int)$option_value['sort_order'] . "'");
		...
		if (isset($data['option_value'])) {
			foreach ($data['option_value'] as $option_value) {
				if ($option_value['option_value_id']) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "option_value SET option_value_id = '" . (int)$option_value['option_value_id'] . "', option_id = '" . (int)$option_id . "', image = '" . $this->db->escape(html_entity_decode($option_value['image'], ENT_QUOTES, 'UTF-8')) . "',  image2 = '" . $this->db->escape(html_entity_decode($option_value['image2'], ENT_QUOTES, 'UTF-8')) . "', long_name = '" . $option_value['long_name'] . "', sort_order = '" . (int)$option_value['sort_order'] . "'");
	...
	public function getOptionValues($option_id) {
		...
		foreach ($option_value_query->rows as $option_value) {
			$option_value_data[] = array(
				...
				'name'            => !empty($option_value['long_name'])?$option_value['long_name']:$option_value['name'],
				'long_name'       => $option_value['long_name'],
