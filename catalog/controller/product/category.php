	...
	public function index() {
		...
		if ($category_info) {
           ...
           $data['details'] = $this->language->get('details');
			...
			$data['hide_description'] = html_entity_decode($category_info['hide_description'], ENT_QUOTES, 'UTF-8');
			...
			foreach ($results as $result) {
				if ($result['image']) {
					$image = $this->model_tool_image->resize($result['image'], 585, 387);
				} else {
					$image = $this->model_tool_image->resize('placeholder.png', 585, 387);
				}
                ...
                $attr = $this->model_catalog_product->getProductAttribute($result['product_id']);
