...
public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false) {
		...
		if ($order_info) {
			...
			if ($order_info['order_status_id'] && $order_status_id && $notify) {
             ...
             if ($notify && isset($order_info['email']) && !empty($order_info['email'])) {
                    $mail = new Mail();
                    $mail->protocol = $this->config->get('config_mail_protocol');
                    $mail->parameter = $this->config->get('config_mail_parameter');
                    $mail->smtp_hostname = $this->config->get('config_mail_smtp_hostname');
    ...
    public function editOrderAjax($order_id, $data) {
        // Void the order first
        $this->addOrderHistory($order_id, 0);

        $this->db->query("UPDATE `" . DB_PREFIX . "order` SET total = '" . (float)$data['total'] . "', date_modified = NOW() WHERE order_id = '" . (int)$order_id . "'");
    ...
    public function editOrderCustom($order_id, $data) {
        // Void the order first


        $this->addOrderHistory($order_id, 0);

        $this->db->query("UPDATE `" . DB_PREFIX . "order` SET  firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', 
        ...
        payment_city = '" . $this->db->escape($data['city']) . "', 
         payment_country = '" . $this->db->escape($this->getCountry((int)$data['country_id'])) . "', 
         payment_country_id = '" . (int)$data['country_id'] . "', 
         payment_zone = '" . $this->db->escape($this->getZone((int)$data['zone_id'])) . "', 
         payment_zone_id = '" . (int)$data['zone_id'] . "', 
         
         payment_method = '" . $this->db->escape($this->getCustomPaymentMethod($data['payment_method'])) . "', 
         ...
         shipping_country = '" . $this->db->escape($this->getCountry((int)$data['country_id'])) . "', 
         shipping_country_id = '" . (int)$data['country_id'] . "', 
         shipping_zone = '" . $this->db->escape($this->getZone((int)$data['zone_id']))  . "', 
         shipping_zone_id = '" . (int)$data['zone_id'] . "', 
         buy_status_id = '" . (int)$data['buy_status_id'] . "', 
         
     
         shipping_method = '" . $this->db->escape($this->getCustomShippingMethod($data['shipping_method'])) . "', 
         
         shipping_code = '" . $this->db->escape($data['shipping_method']) . "', 
         comment = '" . $this->db->escape($data['comment']) . "', 
    ...
    public function getCustomShippingMethod($code)
    {
        $method = '';
        if($code == 'novaposhta.novaposhta'){
            $method = 'Отправка службой "Новая почта"';
        }
    ...
    public function getCustomPaymentMethod($code)
    {
        $method = '';
