...
class ModelReviewStoreReview extends Model
{
    public function addReview($data)
    {
        ...
        $this->db->query("INSERT INTO " . DB_PREFIX . "review SET author = '" . $this->db->escape($data['name']) . "', email = '" . $this->db->escape($data['mail']) . "', link = '" . $this->db->escape($data['link']) . "', customer_id = '" . (int)$this->customer->getId() . "', product_id = '0', text = '" . $this->db->escape($data['text']) . "', date_added = NOW()");

        $review_id = $this->db->getLastId();
		
		if(isset($data['rating']) && !empty($data['rating'])) {
        $this->db->query("UPDATE " . DB_PREFIX . "review SET rating = '" . (int)$data['rating'] . "' WHERE review_id='" . $this->db->getLastId() . "'");
		}
            ...
            $message .= sprintf($this->language->get('text_name'), $this->db->escape(strip_tags($data['name']))) . "\n";
            if ($data['rating']) {
    ...
    public function getReviews($start = 0, $limit = 20)
    {
        ...
        $query = $this->db->query("SELECT r.review_id, r.customer_id,r.email,r.author,r.link,r.admin_answer, r.rating, r.text, r.date_added FROM " . DB_PREFIX . "review r WHERE r.product_id = '0'  AND r.status = '1'  ORDER BY r.date_added DESC LIMIT " . (int)$start . "," . (int)$limit);
