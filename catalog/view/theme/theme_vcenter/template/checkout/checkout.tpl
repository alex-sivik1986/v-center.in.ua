<?php echo $header; ?>
   ...
   <section id="bread-crumbs">
                    <div class="container-1300">
                    	<ul>
                            <li><a href="#"><?=$text_home?></a></li>
                            <li><span>-</span></li>
                            <li><?=$text_chechout_order?></li>
                        </ul>
                    </div><!-- .container -->
                </section>
...
<div class="container-1300">
	<div class="anim-top start-anim">
      ...
      <div id="content" class="c-flex flex">
                       <div class="basket-block">
					   <!----------LOGIN------------->
<div style="display:none" class="panel-collapse collapse" id="collapse-checkout-option">
...
<div id="cart_prod"></div>
					  ...
					  <div class="call-center fadeEl">
                                <div class="img"><img src="img/ico/call-center.svg" alt=""/></div>
                                <?=$text_inform?>
                            </div>
                        
                        </div>
             <div id="checkout" class="fadeEl fadeEl-right start-anim">
                            <form>
                                <div class="item">
                                    <div class="tit">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input id="first" name="firstname" value="<?=$firstname?>" class="form-control" placeholder="<?=$entry_firstname_checkout?>" type="text"><div class="alert"></div>
                                    </div>
                                    <div class="form-group">
                                        <label><?=$text_phone?></label>
                                        <input id="phone" value="<?=$telephone?>" name="phone" class="form-control" placeholder="<?=$entry_phone_checkout?>" type="tel"><div class="telephone"></div>
                                    </div>
                                    <div class="form-group">
                                ...
                                <div class="item">
                                    ...
                                    <div id="collapse-shipping-address" class="delivery-new">
                                ...
                                <div class="item">
                                    <div class="tit">
                                        <div class="img"><img src="img/ico/wallet.svg" alt="" /></div>
                                        <h2><?=$text_payment?></h2>
                                    </div>
                                ...
                                <div class="bnt-block">
								<div style="display:none"  id="collapse-checkout-confirm">
           
          </div>
                                    <button id="confirm" class="btn"><?=$entry_confirm?></button>
                                </div>
                            </form>
                        </div><!-- #checkout -->
                    </div>
...
$.ajax({
			...
			success: function(html) {
				...
				if ($('.checkbox.delivery-post input').is(':checked')) {
...
$.ajax({
		type: 'post',
		url: 'index.php?route=checkout/checkout/validate_this_input',	
		data: $('#checkout input[name=\'firstname\'], #checkout input[type=\'tel\']'),
		dataType: 'json',
		beforeSend: function() {
		...
		success: function(json) { 
if(json['success']) {
	if($( "#shipping-existing" ).hasClass( "active" )) { 
		var address_deliv = $('#shipping-existing select[name="address_id"], #checkout input[name=\'firstname\'], #checkout textarea, #checkout input[type=\'tel\'], #checkout input[name=\'comment\'], #checkout input[type=\'text\']'); 
	} else if($( "#shipping-new" ).hasClass( "active" )) { 
		var address_deliv = $('#shipping-new select, #checkout input[name=\'firstname\'], #checkout input[type=\'tel\'], #checkout textarea, #checkout input[type=\'text\']'); console.log('new');
	}
	$.ajax({
		type: 'post',
...
//-->
			...
			$('.alert').replaceWith('<div style="color:red" class="alert">' + json['errors']['firstname'] + '</div>');
			$('html, body').animate({
			...
			if(json['errors']['phone']) {
				...
				$('.telephone').show();
				$('.telephone').replaceWith('<div style="color:red" class="telephone">' + json['errors']['phone'] + '</div>');
				$('html, body').animate({
				scrollTop: $("#checkout").offset().top
			}, 2000);
			}
...
if(json['success']) {
	$.ajax({
		...
		url: 'index.php?route=checkout/confirm',
		cache: false,
		data: $('#collapse-shipping-address select, #checkout input[name=\'firstname\'], #checkout input[type=\'tel\'], #checkout textarea, #checkout input[type=\'text\']'),
		dataType: 'html',
		beforeSend: function() {
			$('#confirm').button('loading');
		},
