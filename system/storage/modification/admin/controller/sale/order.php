...
//        $data['catalog'] = isset($this->request->server['HTTPS']) && (($this->request->server['HTTPS'] == 'on') || ($this->request->server['HTTPS'] == '1')) ? HTTPS_CATALOG : HTTP_CATALOG;
        ...
        $data['add_ajax'] = $this->url->link('sale/order/add_ajax', 'token=' . $this->session->data['token'], true);
...
//            'address_2' => '',
//            'postcode' => '54000',
//            'city' => 'Отделение №9 (до 30 кг на одно место): ул. Задворецкая, 1а',
//            'zone_id' => '200419',
//            'zone' => 'Луцк',
//            'zone_code' => '1',
//            'country_id' => '300002',
//            'country' => 'Волынская область',
//            'iso_code_2' => 'UA',
//            'iso_code_3' => 'UKR',
//            'address_format' => '',
//            'custom_field' => array(),
//        );
        ...
        $data['payment_codes'] = array('cod','bank_transfer');
        ...
        $data['shipping_methods'] = array(
            ...
            [
                'shipping_code' => 'novaposhta.novaposhta',
                'shipping_method' => 'Отправка службой "Новая почта"',
            ]
        );

        $data['shipping_codes'] = array('pickup.pickup','novaposhta.novaposhta');
		...
		foreach ($results as $result) {
            $order_custom_quantity = $this->model_sale_order->getTotalOrderProductsByOrderId($result['order_id']);
//			$products[] = '';
//
//		    $products = $this->model_sale_order->getOrderProducts($result['order_id']);
//
//			foreach ($products as $key=>$product) {
//				$products[$key]['option'] = $this->model_sale_order->getOrderOptions($result['order_id'], $product['order_product_id']);
//			}
...
//                    'sku'    		   => $product['sku'],
            ...
            $vouchers = $this->model_sale_order->getOrderVouchers($result['order_id']);
            ...
            if($order_info['payment_country_id']){
                $order_zones = $this->model_localisation_zone->getZonesByCountryId($order_info['payment_country_id']);
            }

            $order_cities = array();
            if($order_info['payment_zone_id']){
//                $order_zones = $this->model_localisation_zone->getZonesByCountryId($order_info['payment_country_id']);
                $order_cities = $this->model_localisation_city->getCitiesByZoneId($order_info['payment_zone_id']);
            }
...
//				'products'   => $products,
				'buy_id'    => $result['buy_status_id'],
				'zone'      => $result['shipping_zone'],
				'order_custom_quantity'  => $order_custom_quantity,
...
//
        ...
        $data['buy_statuses'] = $this->model_sale_order->getBuyStatuses();
	...
	public function getFormAjax() {
		...
		$this->response->setOutput($this->load->view('sale/order_form_ajax', $data));
	}
